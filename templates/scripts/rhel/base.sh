...
delete_locale(){
  ...
  \cp -rf /usr/lib/locale/locale-archive /usr/lib/locale/locale-archive.tmpl
  build-locale-archive  # 如果使用上面的 mv 语句再加上此命令会导致用户退出当前 shell，但用 cp 语句与此命令就不会退出当前 shell,参考：https://mlog.club/article/4495255
